<script lang="ts">
	...
	async function saveUser() {
		...
		if (!isEditing && !password) {
			errorMsg = 'Password is required for new users.';
			return;
		}

		const path = isEditing ? `/api/users/${editingId}` : '/api/users';
				...
				>
					...
					<div class="actions">
						...
						<div class="submit-wrap">
							<Button type="submit" text={isEditing ? 'Update User' : 'Register User'} />
						</div>
					</div>
				</form>
			</div>
