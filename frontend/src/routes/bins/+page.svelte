<script lang="ts">
	...
	async function saveBin() {
		...
		const path = isEditing ? `/api/warehousebins/${editingId}` : '/api/warehousebins';
				...
				>
					...
					<div class="actions">
						...
						<div class="submit-wrap">
							<Button type="submit" text={isEditing ? 'Update Bin' : 'Save Bin'} />
						</div>
					</div>
				</form>
			</div>
					...
					{:else if bins.length === 0}
						<tr
							><td colspan="6" class="empty-state"
								>No bins yet. Create one above - stock cannot be received until a bin exists.</td
							></tr
						>
					{:else}
