	...
	>
		...
		{#if !isLoadingOptions && binOptions.length === 0}
			<div class="alert alert-info">
				No warehouse bins exist yet. Create one on the <a href={resolve('/bins')}>Bins</a> page before
				receiving stock.
			</div>
		{:else if !isLoadingOptions && itemOptions.length === 0}
			<div class="alert alert-info">
				No items exist yet. Define one on the <a href={resolve('/inventory')}>Master Inventory</a> page
		...
		<div class="submit-row">
			<Button type="submit" text="PROCESS ARRIVAL" {isLoading} loadingText="RECEIVING STOCK..." />
		</div>
	</form>
</div>
